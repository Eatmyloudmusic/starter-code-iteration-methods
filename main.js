...
//comes back as TRUE
const ovationProperty = users.filter(user => user.company === 'OVATION')
...
const firstOver38 = users.filter(user => user.age === 38)
...
const isActiveAgeOver38 = (users.filter(user => user.age > 38)).find(user => user.isActive === true)
...
const balanceAtZenco = (users.filter(user => user.balace)).find(user => user.company === 'ZENCO')
...
const ageWithFugiat = (users.filter(user => user.tag === 'fugiat')).includes(user => user.age)
